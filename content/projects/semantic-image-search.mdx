...
Missing, partial, and unavailable data each get a state that names which one it is, so an incomplete view of the library never reads as a complete one.
...
<AsciiScreen label="Screen 1, cold start: window with an empty Layers panel reading 'No layer packs yet' and a search pane prompting to open a folder; no result count is drawn.">
...
<AsciiScreen label="Screen 2, indexing: a determinate progress bar reading 'Embedding 812 of 1,284' under the search field, above a two-row grid of photo placeholders.">
...
Each condition below is worded for its cause and offers a way out of it. Two rules from [Heuristics Canon](https://github.com/darce/heuristics-canon) decided the split: [CAL-02](https://github.com/darce/heuristics-canon/blob/main/lexicons/ml-systems.md#cal-02), which holds that abstention is a valid result rather than a forced classification, and [RLSE-04](https://github.com/darce/heuristics-canon/blob/main/lexicons/engineering.md#rlse-04), which treats an undesigned state as a bug. Six states got copy of their own because those two rules said an undifferentiated blank was the defect, not the fallback.

No folder yet is the start state, held apart from an empty result for the reason Screen 1 gives.
...
<AsciiScreen label="Screen 7, timeline: an all-dates histogram 2019 to 2025 on a square-root count scale with 94 undated photos held off the axis and 1,190 dated; a July 2021 daily zoom; and a truncated view offering a 'Before 1962' action.">
