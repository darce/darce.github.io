...
> **About the screens.** The ASCII screens are drawn from the shipped code as it stands at 0.2.3, the release linked above. A test in the product repository pins each quoted string: it must occur verbatim in the named Swift file, so a drawing that stops matching the app fails a build rather than ageing quietly. The drawings are the checkable contract. The pixel screenshots come from the 0.2.0 build (27 August 2026) and have not been retaken. Their window title still reads Semantic Image Search, the product's working name; the rename landed in 0.2.1. A screenshot is faithful to the build it came from and a drawing tracks the code as it now stands, so the two disagree on purpose — about the title, and about the thumbnail states in Screen 3, which 0.2.3 added.
...
**[Download Visual Search Workbench 0.2.3 for macOS](https://dl.darce.xyz/)** — Apple Silicon, macOS 14 or later. The link resolves to the current release on GitHub, which serves the file. Take the one asset named `VisualSearchWorkbench-0.2.3.dmg`; the Source code archives beside it are attached by GitHub to every tag and hold no source.

The disk image is about 295 MB. The embedding, segmentation, and detection models are inside it, so nothing is fetched on first launch and the app has no reason to open a socket.

Two things to know before you download, both of which follow from this being independent work rather than a shipped commercial product:

- **It is not notarized by Apple.** Notarization needs a paid developer membership this project does not hold, so macOS will refuse the first double-click. Apple's own override — System Settings, Privacy & Security, *Open Anyway* — opens it, once. If you would rather not run software Apple has not checked, that is a reasonable place to stop.
- **Check the bytes.** Each release publishes a `SHA256SUMS` file beside the image; `shasum -a 256 -c SHA256SUMS` confirms you received what was built. GitHub serves the download, not this site, and the digest is the only end-to-end check across that hop.

Both caveats, and the licences of the bundled models, are written out on the [download page](https://github.com/darce/visual-search-workbench).
...
**Screen 1 — Cold start.** Before a folder is chosen, Search says "Choose a folder to search. Your photos stay where they are." Add Folder… sits directly under the query, the one way in. The left pane is Assets — the photos you have matted, each owning its layers — and it says so when it is empty rather than showing a bare frame. No zero-result count is drawn.
...
**Screen 2 — Indexing.** A determinate meter reports "Embedding 812 of 1284…" above a two-row photo grid. Add Folder… stays put while it runs: a job in progress is not a reason to hide the way in.
...
**Screen 3 — Semantic results.** This screen shows a semantic query for a red bicycle and a grid of 64 nearest results, mid-decode. Four thumbnails decode at a time and a tile stops waiting after two seconds, so on first paint most tiles are still placeholders. Each one asks again on a jittered backoff. A tile whose file is genuinely gone stops asking and says "Picture unavailable" — grey means working, and a frame still claiming to work half a minute later would be lying.
...
**Screen 4 — A photo selected, details open.** IMG_4417.HEIC is selected; double-click opens it. The details panel shows size (4032 × 3024), camera (Apple iPhone 13 Pro), lens (26 mm f/1.5), capture time (2021:07:14 18:22:00), and GPS (64.1466°, -21.9426°) above an offline mini-map. The map is drawn from the bundled coastline, and the two lines under it say where on Earth this is and how wide the window is, because a grid with no stated scale cannot be read.
...
Each label sits with its value. The panel keeps its open or closed state as the selection changes. The mini-map draws an offline graticule and the bundled Natural Earth coastline, and never fetches tiles. Where the shipped coastline fails to load it says so, because a blank shoreline reads as an inland photograph.
...
The map is drawn for selection, and its geometry stays inside what a projection can say. A UMAP layout preserves neighbourhoods rather than distances ([McInnes, Healy & Melville 2018](https://arxiv.org/abs/1802.03426)). So the surface has no axes and no distance readout, and a gap between two clusters is not a fact about the library: claims from the layout stop at local contiguity.
...
- Every tile arrives, or says why not. A grid of 64 results decodes four thumbnails at a time and a tile stops waiting after two seconds, so most tiles are still grey on first paint. A tile that lost its place in that queue asks again, on a spread-out backoff so the grid does not retry in lockstep, rather than staying grey for as long as it is on screen. A photo whose file is genuinely gone ends up marked unavailable. Grey means working, and a frame still claiming to work half a minute later would be lying. Fixed in 0.2.3; the retry schedule and the wiring are both held by tests.
...
How the product was designed and reviewed, from the whole-product UX map to the rule corpus that vetted each screen, is its own article: [A Map and a Canon](/research/visual-search-workbench-process/).
