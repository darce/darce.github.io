...
**[Download Visual Search Workbench 0.2.2 for macOS](https://dl.darce.xyz/)** — Apple Silicon, macOS 14 or later. The link resolves to the current release on GitHub, which serves the file.

The disk image is about 298 MB. The embedding, segmentation, and detection models are inside it, so nothing is fetched on first launch and the app has no reason to open a socket.
