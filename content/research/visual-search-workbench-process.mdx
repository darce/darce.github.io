...
> **What this is:** the working method behind [Visual Search Workbench](/projects/semantic-image-search/), a native macOS app that searches photo folders by text or by picture, named Semantic Image Search through its 0.2.0 build. That page shows what shipped. This one shows how it was designed and reviewed.
...
The map was written before the interface existed, which means the built interface could still betray it. So after the screens were running, the critique pack ran again, this time holding the canon's interaction rules against the code itself. One late pass caught three defects that pixel-level review had walked past.

A cancel button lied. While the search model loaded, a progress banner offered Cancel, but pressing it could not stop the load it appeared to govern. Worse, the handler was borrowed from the indexing banner, so it wrote a cancelled entry into the job history for an indexing run that had never started. The interface was fabricating a past.

A fresh launch showed two explanations of the same empty screen at once: an invitation to choose a folder stacked above a preparing-search meter. Each message came from its own independent condition, so nothing prevented both from being true together. The fix was structural rather than cosmetic: one state now decides what the empty screen is, instead of parallel conditionals each deciding for themselves.

And two visually identical progress banners could stack, each with its own Cancel. Their accessibility names told them apart, so a screen-reader user could tell which Cancel stopped which job while a sighted user could not.

Each finding cites the file and lines it checked, states which claims were verified by reading the code, and flags what was asserted without measurement. All three were fixed before release. The lesson repeats the map's founding one: interface states written as independent conditionals will eventually all be true at once, and only an inventory of states catches that before a person does.
