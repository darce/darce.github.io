...
type: 'method'
status: 'running'
description: 'Process notes from building Semantic Image Search: inventory every screen and state before code, then hold each one against decision rules distilled from engineering and machine-learning sources.'
details: 'Process notes from building Semantic Image Search. A text-first UX map inventoried every screen, state, and flow before interface code existed, and a source-linked rule corpus reviewed each one. The same retrieval discipline governed the product and the tools that built it.'
links:
  - url: https://github.com/darce/heuristics-canon
    label: Heuristics Canon on GitHub
thumbnail:
  src: work/sis-search-layer.png
  alt: "Semantic Image Search ranking a film-scan library for the query 'flowers': a grid of pink cosmos photos with the selected photo's details open beside it."
  position: center
tags:
  - Process
  - Interaction Design
...
> **What this is:** the working method behind [Semantic Image Search](/projects/semantic-image-search/), a native macOS app that searches photo folders by text or by picture. That page shows what shipped. This one shows how it was designed and reviewed.
...
That convergence is the finding these notes record. Rules written for databases and servers produced the product's caveat states, and the same rules kept the build tooling honest about its own index. A derived index, whatever it indexes, must say how much of its corpus it can see.
