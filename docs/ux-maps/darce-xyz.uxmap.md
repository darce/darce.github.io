...
- Model the repositioning to 'Design Technologist': four-item nav, no /practice hub, semantic-image-search case first, heuristics canon filed under research, no résumé surface until a current one ships
...
```mermaid
flowchart TD
  %% flow: Land → SIS case → heuristics canon → contact job=job-assess-design-judgment
  n_landing["Home (screen)"]
  n_project_detail["Project case (screen)"]
  ...
  n_exit_mailto["Mail client (exit)"]
  n_research_detail -->|heuristics-canon exhibit| n_exit_mailto
```
...
- Resolved: the footer ships. It is the only in-site link to /privacy/. It renders on every Layout page, and every route now uses Layout. ([NAV-08][A11Y-04])
...
- Resolved: the résumé PDF was removed as stale — it still read "Senior Front-End Developer" and carried the causal $9.2M claim the site had already corrected. There is no résumé surface until a new one ships; About and the footer offer contact instead. A test now fails if any MDX links a root-relative file that is not in public/. ([WRIT-41][FORE-06])
...
- Shipping a résumé surface before there is a current résumé to ship
...
| ...end of main content                                           |
+==================================================================+  <- 2px t('link') rule
| Daniel Arcé                              [Privacy] | [Contact]  |  band = t('masthead')
+------------------------------------------------------------------+
```
...
- The footer is outside `#main-content`, so the global link colour in
  ...
  ([A11Y-06]).
- `Footer` returns `null` when no provider supplies data. Every route now uses
  `Layout`, so that branch is a guard rather than a state the site renders in.
